...
| `voice_studio.batch` | Чиста модель черги файлів: явні переходи pending→running→done/failed/skipped, пауза, стеля 500 елементів, без потоків і без доступу до джерел |
| `voice_studio.smart_text` | Абзаци за паузами, часові мітки, ручні мітки спікерів у metadata, рендер Markdown/plain |
| `voice_studio.sync_folder` | Локальне дзеркало транскриптів (Markdown + JSON, опційно аудіо) у папку користувача: валідація кореня, атомарні записи, ніколи не видаляє |
| `voice_studio.audio_errors` | Зрозумілі повідомлення про відсутній аудіопристрій замість сирих помилок PortAudio |
...
- Одночасний запуск двох транскрибувань UI блокує; черга використовує той самий
  одиночний job path і ніколи не запускає другу задачу паралельно.
- Дзеркалення в папку синхронізації виконується після кожного запису в сховище
  (job done, збереження правок, мітка спікера, застосування cleanup); корінь
  перевіряється повторно перед кожним записом, а помилка потрапляє лише в рядок
  стану. «Синхронізувати все зараз» працює на окремому GUI worker-і.
