...
| SEC-001 | **Closed headless (W2-M2).** PyAV moved to a disposable spawn child with a hard deadline; the parent no longer imports `av` at all, asserted by the runtime-boundary test. ffmpeg leads its own process group and its descendants are killed with it — verified RED by reducing the kill to the direct child, which leaves a grandchild alive. Containment failure is fatal, never a fallback to in-process parsing. Ceilings: 2 GiB source (before the parser starts), 7,200 s duration, 230,404,096-byte output via ffmpeg `-fs` plus an after-check, bounded converter diagnostics. Windows process-group behaviour executes in CI but has no physical acceptance, so it is `IMPLEMENTED_PENDING_NATIVE_ACCEPTANCE`. | `src/hermes_voice_studio/media.py`, `tests/test_media_containment_app.py`, `tests/test_runtime_boundaries_app.py` |
| REL-002 | **Closed.** The residue was wiring, not enforcement: the probe now takes its deadline from the operation budget instead of only its own cap, and `jobs.py` supplies `MAX_SOURCE_BYTES`, which `import_source` has always enforced mid-stream but nothing ever passed. | `src/hermes_voice_studio/{service,jobs}.py` |
