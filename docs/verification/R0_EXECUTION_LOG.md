...
| W2-E1 Slice B2 encrypted v2 verification | `4641c92` | K3 RED 51 failed / 1 legacy-v1 pass; controller line-by-line review plus RED regressions for boolean version confusion, non-object and invalid-UTF-8 manifests, empty wrong passphrase classification, nonportable/colliding encrypted logical names and invalid-UTF-8 private index; create/verify/crypto/v1-backup/archive-safety gate; compileall; Ruff; `pip check`; staged diff and secret/private-path scan | related 185 passed, 2 Windows symlink-privilege skips in 33.07s with no warnings; v1 dispatch remains compatible; v2 validates structure/KDF bounds before derivation, authenticates manifest then index, rejects portable path aliases, and streams every payload once into a discard sink; baseline before the slice was 690 passed / 9 Windows skips; full post-change suite deferred by the W2-E1 slice plan | `backup.py` version dispatch/v2 verification, archive portable-name rules or budgets, backup crypto, v2 create format, or corresponding tests change; Slice C1 must reuse the authenticated context without a second full decrypt, and the full source gate remains due after Slice D |
| W2-E1 Slice C1 encrypted v2 data restore | `fea4965` | K3 RED 23 failed / 5 early-path passes; controller review plus RED/GREEN regressions for pre-authentication destination-parent mutation, unauthenticated unreferenced audio and recovery when the destination never existed; restore/verify/create/crypto/v1-backup/archive-safety/storage gate; compileall; Ruff; `pip check`; staged diff and added-line secret/private-path scan | related 279 passed, 6 Windows symlink-privilege skips in 41.74s; v2 decrypts each mapped payload exactly once into staging or a discard sink, journals before the first plaintext disk write, preserves originals/models/exports and safely discards an interrupted contained staging even when no prior live root existed; baseline before the slice was 749 passed / 9 Windows skips; full post-change suite deferred by the W2-E1 plan | `backup.py` v2 restore/context/journal/recovery flow, storage restore/audit behavior, backup crypto, archive validation or corresponding tests change; Slice C2 must rerun this boundary while adding encrypted settings recovery, and the full source gate remains due after Slice D |
| W2-E1 Slice C2a encrypted settings recovery | `a2642c7` | K3 RED 19 failed / 2 early-path passes; controller line-by-line review plus seven RED/GREEN regressions for ordinary post-swap settings failure, retry after recovery-promoted `swap_started`, death before the `swap_completed` journal write, root-level sidecar extras, authenticated invalid manifest profiles, empty pending settings targets and death before sidecar cleanup; settings-recovery/restore/verify/create/crypto/v1-backup/archive-safety/storage gate; compileall; Ruff; `pip check`; staged diff and added-line secret/private-path scan | related 306 passed, 6 Windows symlink-privilege skips in 57.85s; v2 sidecar contains only authenticated manifest plus encrypted index/config members, reuses the complete v2 manifest/index validator, preserves retry state across ordinary and hard failures, durably marks applied settings before safe sidecar cleanup, and keeps v1 recovery behavior; baseline before the slice was 779 passed / 9 Windows skips; full post-change suite deferred by the W2-E1 plan | `backup.py` v2 sidecar/settings/journal/recovery flow, backup crypto or related restore/config/storage tests change; Slice C2b must rerun API/CLI recovery boundaries, and the full source gate remains due after Slice D |
| W2-E1 Slice C2b encrypted backup CLI | `ccdb9d1` | K3 RED 13 failed / 5 existing-path passes; controller review plus RED/GREEN regressions for blocking a new restore after any failed journal settlement and treating missing stdin as a concrete noninteractive error; CLI/backup-settings-recovery/restore/verify/create/v1-backup gate; compileall; Ruff; `pip check`; staged diff and added-line secret/private-path scan | related 205 passed, 2 Windows symlink-privilege skips in 51.77s; `backup create --encrypt` prompts twice, v2 verify/restore and pending recovery prompt once through terminal-only `getpass`, v1 never prompts, no argv/environment passphrase channel exists, and failed/cancelled recovery preserves state and blocks replacement; baseline before the slice was 806 passed / 9 Windows skips; full post-change suite deferred by the W2-E1 plan | `cli.py` backup parsing/prompt/settlement flow, backup/recovery APIs or related CLI tests change; Slice D must cover GUI/startup integration and secret-hygiene/package proofs, with the full source gate after its final production change |
| W2-E1 Slice D1 encrypted backup GUI | `00d7592` | K3 RED 20 failed / 2 existing-path passes; controller line-by-line review plus a RED/GREEN regression that removes the create passphrase from the retained backup callback closure; GUI/startup/i18n and worker-registry contract gate; compileall; Ruff; `pip check`; one final full source gate after the last D1 production change; staged diff check | GUI/contract boundary 80 passed in 1.99s; full source gate 849 passed, 9 Windows symlink-privilege skips in 93.26s; encrypted create is explicit opt-in with two masked prompts, verify/restore prompt once on the Tk thread, cancel is non-destructive, interrupted settings recovery can resume at startup, and no passphrase is stored in app state or posted events | `app.py`, backup GUI/startup recovery, i18n catalogs or corresponding tests change; Slice D2 must still prove repository-wide secret hygiene, source frozen-style cryptography, deterministic SBOM/structure, documentation alignment and security self-review; do not repeat the full source gate unless D2 changes production code |
| W2-E1 Slice D2a automated security/package proofs | `bbdc990` | K3 added source-level proofs for design items 17–20; controller review found that the initial hygiene test omitted the real settings files and exported diagnostics, then proved the strengthened scanner with a deliberate leak mutation (expected FAIL) before the clean GREEN run; two CLI noninteractive tests were made independent of pytest capture mode after reproducing their `-s` failure; compileall; Ruff; `pip check`; focused backup/security/SBOM/package gate; post-commit Git-tracked scan | focused 237 passed in 56.27s; post-commit tracked-file proof 1 passed and explicit non-TTY proofs 2 passed under `pytest -s`; deterministic SBOM has 59 components, 11,348 bytes and SHA-256 `5f60473d50bff64a0a56036b0d3ab3f7fc8b6a68c4613ea1af0f44f866252e7a`; source settings, pending/restored settings, restored dictionary, journal, encrypted sidecar, exported diagnostics, API/CLI output and tracked non-test files contain no cycle secrets; structure/chunk matrix and source cryptography round-trips pass | test-only increment: reuse D1 full source gate 849 passed / 9 Windows skips while production remains unchanged; real PyInstaller/frozen executable proof remains `NOT_RUN` for R0.10; D2b must align product/security/help/status documentation and perform the final security self-review |
| W2-E1 Slice D2b documentation/security closeout | `a2287b4` | K3 aligned README, architecture, security, roadmap/status, verification and uk/cs/en Help; controller and an independent read-only reviewer checked claims against `backup.py`, `backup_crypto.py`, CLI/GUI and the design, correcting the current SBOM count, whole-archive-vs-per-chunk authentication wording and managed-audio default in all Help locales; final reviewer verdict CLEAN with no Critical/Important findings; Help validation; compileall; Ruff; `pip check`; focused documentation contracts; final full source gate; diff and secret/private-path scan | Help 13 Markdown files PASS; focused 128 passed; final full source gate 859 passed, 9 Windows symlink-privilege skips in 107.64s; current SBOM documented as 59 components / 11,348 bytes / SHA-256 `5f60473d50bff64a0a56036b0d3ab3f7fc8b6a68c4613ea1af0f44f866252e7a`; source/headless W2-E1 is complete and documentation now distinguishes structural errors, per-chunk staging authentication, logical-name confidentiality and historic W4-B1 evidence | any backup/crypto/CLI/GUI/i18n/Help/security/status or dependency-lock change; real PyInstaller packaged probe and native acceptance remain `NOT_RUN` for R0.10 |
