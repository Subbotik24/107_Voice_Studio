...
### Increment 13 — deep program audit and remediation

- Base: `ca196ff`. Audit run with the repository's `deep-program-audit` skill: three independent read-only passes (UI lifecycle/state transitions; persistence, calculations and invariants; runtime contracts and adversarial boundaries) followed by a coordinator skeptic pass against the code. Full suite was green before the audit (1161 passed / 14 skips), i.e. every confirmed defect below was uncovered by tests.
- Confirmed and fixed (each with a failing-first regression test): (1) a dictionary rule with an empty replacement plus any editor save silently lost immutable segment raw text — `sync_segments` now absorbs non-editable segments into a neighbour and `update_editor_state` enforces the joined-raw-text guard except for a deliberate full clear; (2) the filler-word dialog could apply stale offsets to a transcript swapped in by a finishing job — refused via a snapshot check (`editor_filler_stale`); (3) `_show_page` ran the destructive editor/dictionary/settings guards before a precondition that could still veto navigation — precondition hoisted; (4) restore while the Settings page was open let Save overwrite restored settings and re-armed the hotkey on the capture page — page rebuilt, Help reset on language change; (5) Ollama discovery silently overwrote an in-progress model choice and desynchronised the unsaved-changes baseline — programmatic sets limited to untouched fields with the baseline advanced; (6) the status bar lived inside the Studio page so feedback was invisible on five of six pages — moved to the workspace level; (7) one unparsable payload row crashed `list()`, `audit()` and backup creation — skipped and counted (`invalid_records` in the audit result); (8) closing the window discarded unsaved Settings edits — guard added; (9) launchers: credential prompts could hang forever (`GIT_TERMINAL_PROMPT=0`), a failed checkout aborted the macOS launch or launched a half-updated Windows tree (best-effort fallback in both), dependencies were never reinstalled after an update (reinstall when HEAD moved), local edits discarded without a warning (warning added); an executable test drives the macOS update block against a throwaway repository lacking `origin/main`; (10) `AvPcmSource` ignored `stream.start_time` so segment playback of offset streams started early and duration was inflated — seek/lead/duration corrected, `time_base` None guarded, real-media tests added; (11) the 240 s Ollama request ceiling was reported as "runtime unavailable" — distinct timeout message, `audio_chat` accepts a timeout; (12) hardware-detection fallback `KeyError('detail')`, CR/tab/NBSP whitespace in filler removal, stale confidence rows after an invalid threshold, playback continuing after deleting the playing transcript, and an uncancelled deferred hotkey restart.
- Verified clean by the audit: all 14 Dashboard metrics match an independent recomputation on a 13-row synthetic set; user originals are never deleted; the proxy-free opener covers every Ollama endpoint; SQL is parameterised; no Ukrainian residue in cs/en catalogs; the playback controller survives 200 interleaved seeks without leaks or duplicate sinks.
- Deferred product decision (not patched): CLI `--engine openai-cloud` derives `offline_only=False` from the profile and its dedicated guard is unreachable, while CLAUDE.md states `offline_only` blocks all cloud activity; a test currently locks in the override.
- Controller full gate on the remediated tree: compileall PASS; Ruff PASS; Help validation PASS (13 Markdown files); `bash -n run_mac.command` PASS; pytest PASS 1194 passed / 14 Linux junction skips; wheel build PASS; `pip check` PASS; `git diff --check` PASS.
- Real Windows/macOS launcher runs, a real audio device, a real Ollama runtime and native acceptance: NOT RUN here.
