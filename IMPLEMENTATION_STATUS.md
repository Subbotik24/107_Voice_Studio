...
| W1.5 evidence base | PASS | suite cannot hang (`pytest-timeout`, 120 s/test); CI compiles and lints the same `src tests scripts packaging` surface as the local gate; all three workflows (CI, Test RC release, CodeQL) bounded at 20 minutes |
...
| Usability 10–12 settings profile UX, in-window Settings/Help, launchers (2026-09-01) | PASS source/headless | Ollama discovery falls back to all installed models with an explicit no-audio warning; Settings and Help are central pages with an unsaved-changes guard; the loopback Ollama client bypasses system proxies; launchers always run the checked-out source tree; gates `1145`, `1158`, `1160 passed` with 14 Linux junction skips |
| Deep audit and remediation round 2 (2026-09-01) | PASS source/headless | twelve confirmed findings fixed with failing-first tests (segment raw-text loss on empty replacement, stale filler offsets, navigation guard order, restore over the Settings page, discovery overwriting an in-progress choice, workspace-level status bar, unparsable payload rows, close discarding Settings edits, launcher fallbacks, PyAV stream start offset, Ollama timeout wording, detection fallback); gate `1194 passed` |
| Feature pack: batch queue, Smart text, sync folder, Dashboard dynamics, seek slider, status-bar progress (2026-09-01/02) | PASS source/headless | pure `batch.py`/`smart_text.py`/`sync_folder.py` modules plus Studio queue panel, Smart text tab with speaker labels in metadata only, Settings → Synchronisation with root validation, 14-day activity and distribution canvases, position slider, slim progress bar; gates `1323` and `1376 passed`; real multi-file engine run, real cloud-sync client and real audio device NOT RUN |
| Real-runtime smoke on the cloud VM (2026-09-02) | PASS headless real store | real launcher run under Xvfb, real `sounddevice` without devices, queue of two real WAV files against an absent Ollama runtime, sync into a real temporary folder, editor re-mirror, playback without an output device; three fixes from it (transport bar always visible, plain queue error text, clear no-device wording); gate `1379 passed` with 14 Linux junction skips |
| R0.11 audit fixes (2026-09-02) | PASS source/headless | sync mirror re-validates its root against the data folder before every write and never creates the root; Save stores the resolved absolute sync path; speaker labels are written under one `BEGIN IMMEDIATE` transaction; launcher self-update is opt-in via `VOICE_STUDIO_AUTO_UPDATE=1` and never overwrites local edits; CodeQL bounded and pinned to 4.37.9; Linux CI job with Tk under Xvfb; Windows build writes a release manifest when an acceptance result is given; one version-consistency test |
...
- restoring the batch queue after a restart, queue scheduling budgets and
  per-item retry (the in-memory queue itself shipped on 2026-09-02);
