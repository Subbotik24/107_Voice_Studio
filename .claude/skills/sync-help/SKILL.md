...
3. Map each user-visible change to affected topics with targeted `rg` searches.
   ...
   the running app too.
4. Map each affected topic slug across `docs/help/uk/`, `docs/help/cs/`, and
   `docs/help/en/`. Update that slug in all three locales with the same verified
   behavior and each locale's exact UI labels. Update a screenshot only when
   its visible text, controls, layout, or documented state actually changed.
5. Run:
...
- Ukrainian, Czech, and English topic slug/order coverage must remain equal.
  `scripts/check_help.py` is the required parity/link gate; a missing locale is
  a failed sync, not an optional translation follow-up.
...
- Keep Ukrainian, Czech, and English public README links aligned when
