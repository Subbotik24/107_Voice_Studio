...
### Batch transcription — розвиток черги

Базова черга файлів у пам'яті вже є в R0 (2026-09, `voice_studio.batch`):
один job за раз на тому самому шляху, що й одиночне розпізнавання. R1 — це її
розвиток, а не нова функція.

- Користь: архіви лекцій, інтерв’ю, медіатеки без нагляду.
- Складність: scheduler, disk/RAM budgets, per-item retry, aggregate progress,
  відновлення черги після перезапуску (persisted queue state).
