...
def registry_url() -> str | None:
    return os.environ.get("HVS_MODEL_REGISTRY_URL", "").strip() or None


def fetch_registry(url: str, *, timeout_seconds: int) -> dict[str, Any]:
    request = urllib.request.Request(url, headers={"User-Agent": "Hermes-Voice-Studio"})
...
def download_asset(
    ...
    *,
    ...
    request = urllib.request.Request(asset["url"], headers={"User-Agent": "Hermes-Voice-Studio"})
    with (
