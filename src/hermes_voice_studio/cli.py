...
def build_parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        prog="hermes-voice",
        description="Local desktop transcription and Hermes Whisper model laboratory.",
    )
    ...
    transcribe.add_argument(
        "--engine", choices=["faster-whisper", "hermes-whisper", "openai-cloud"]
    )
    transcribe.add_argument("--model", help="faster-whisper model name or local model path")
    transcribe.add_argument("--bundle", type=Path, help="trained Hermes .hws bundle")
    ...
    cleanup.add_argument("--provider", choices=["openai"], default="openai")
    cleanup.add_argument("--apply", action="store_true")
    cleanup.add_argument("--allow-cloud-text", action="store_true")
    cleanup.add_argument("--model", help="OpenAI Responses model override")
    ...
    verify = sub.add_parser("verify-model", help="verify a Hermes .hws bundle")
    ...
    benchmark.add_argument("--engine", choices=["faster-whisper", "hermes-whisper"])
    benchmark.add_argument("--model")
    benchmark.add_argument("--bundle", type=Path)
    ...
    model = sub.add_parser(
        "model",
        help="run Hermes model development commands (doctor/train/evaluate/bundle/...)",
    )
    model.add_argument("arguments", nargs=argparse.REMAINDER)
    return root
...
def _load_effective_settings(args: argparse.Namespace) -> Settings:
    ...
    if getattr(args, "bundle", None) is not None:
        overrides["hermes_bundle"] = str(args.bundle.expanduser())
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "cloud":
            ...
            if args.cloud_key_command == "test":
                try:
                    ...
                    raise RuntimeError("Install hermes-voice-studio[cloud] to test OpenAI") from exc
        ...
        if args.command == "verify-model":
            ...
            _json({"status": "PASS", **verify_model_bundle(args.bundle)})
            return 0

        if args.command == "model":
            if not args.arguments:
                parser.error("hermes-voice model requires a Hermes model command")
            from hermes_whisper.cli import main as model_main

            return int(model_main(args.arguments))
