from hermes_voice_studio.diagnostics import diagnostics, module_available
...
def test_module_available_performs_a_real_import(monkeypatch):
    ...
    monkeypatch.setattr("hermes_voice_studio.diagnostics.importlib.import_module", fail_import)
    assert not module_available("tkinter")


def test_diagnostics_exposes_capabilities(monkeypatch):
    monkeypatch.setattr(
        "hermes_voice_studio.diagnostics._microphone_status",
        lambda _available: (True, None),
    )
    monkeypatch.setattr(
        "hermes_voice_studio.diagnostics._hotkey_status",
